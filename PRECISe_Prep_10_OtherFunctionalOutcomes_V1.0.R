...
# If na.rm = TRUE is added, the minimum score is calculated and only rows with exclusively NAs become 0

p$MRCSUMscoreD30 <- rowSums(p[,c(308:319)])   #627 NAs -> 621 are true NA (not done), 6 are missing (at least) one entry  
p$MRCSUMscoreD90 <- rowSums(p[,c(390:401)])   #640 NAs -> 637 are true NA (not done), 3 is missing (at least) one entry
p$MRCSUMscoreD180 <- rowSums(p[,c(472:483)])  #676 NAs -> 671 are true NA (not done), 5 are missing (at least) one entry
...
ggplot(p, aes(x=studyfeed, y=HGS.pred.30, fill=studyfeed)) + 
  geom_boxplot() 

summary(p$HGS.pred.30)
summary(p$HGS.pred.90)
summary(p$HGS.pred.180)

summary(p$MRCSUMscoreD30)
summary(p$MRCSUMscoreD90)
summary(p$MRCSUMscoreD180)

summary(p$MRCSUMscoreD30)
summary(p$MRCSUMscoreD90)
summary(p$MRCSUMscoreD180)

p$MRCbelow46.30 <- ifelse(p$MRCSUMscoreD30 < 46, 1, 0)
table(p$MRCbelow46.30, useNA = "always")

p$MRCbelow46.90 <- ifelse(p$MRCSUMscoreD90 < 46, 1, 0)
table(p$MRCbelow46.90, useNA = "always")

p$MRCbelow46.180 <- ifelse(p$MRCSUMscoreD180 < 46, 1, 0)
table(p$MRCbelow46.180, useNA = "always")


hist(p$MRCSUMscoreD30)
hist(p$MRCSUMscoreD90)
hist(p$MRCSUMscoreD180)
...
## 1.4 Rockwood Clinical Fraily Scale - no adjustments necessary ----

table(p$FRAIL_D30, useNA = "always")
hist(p$FRAIL_D30)

table(p$FRAIL_D90, useNA = "always")
hist(p$FRAIL_D90)

table(p$FRAIL_D180, useNA = "always")
hist(p$FRAIL_D180)
