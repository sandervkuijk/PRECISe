...
## Primary outcome: EQ5D.HUS
class(dl$EQ5D.HUS.imp0)
range(dl$EQ5D.HUS.imp0, na.rm = TRUE) # Is lowest value OK?
hist(dl$EQ5D.HUS.imp0, xlab = "EQ-5D-5L HUS", main = NULL)
sum(!is.na(dl$EQ5D.HUS.imp0))

## Visual inspection of patient-level data
lm.group1 = lmList(EQ5D.HUS.imp0 ~ I(fupday - 30)|Participant.Id,
                   subset = studyfeed == "Group 1", data = dl)
lm.group2 = lmList(EQ5D.HUS.imp0 ~ I(fupday - 30)|Participant.Id,
...
pdf("int_and_slopes.pdf", width = 8, height = 4.5)
op <- par(mfrow = c(1, 2))
...
## Naive modelling, ignoring dependencies in the data
fit_0 <- lm(EQ5D.HUS.imp0 ~ studyfeed*fupday + EQ5D.HUS.proxy.0, data = dl)
summary(fit_0)
...
fit_1 <- lme(EQ5D.HUS.imp0 ~ studyfeed*fupdaym +
...
## Hence, model fit_3b is the final model for the primary outcome,
## but first, check/ test assumptions.
...
pdf("fit_vs_resid_by_group.pdf", width = 8, height = 4.5)
plot(fit_3b, resid(., type = "p") ~ fitted(.) | studyfeed)
dev.off() # No obvious difference in variance between groups

pdf("fit_vs_resid_by_fupday.pdf", width = 8, height = 4.5)
plot(fit_3b, factor(fupday) ~ resid(.), abline = 0, ylab = "")
dev.off()

# Now test homogeneity of variance formally
dl_fitted <- fit_3b$data
keep <- c("Participant.Id", "fupdaym", "EQ5D.HUS.imp0", "Institute",
...
anova(Levene.Model.2) # Some evidence of heteroscedasticity for predictor
                      # follow-up time. Figure above indicates no substantial
                      # deviation
...
pdf("qq_resid_by_group.pdf", width = 8, height = 4.5)
qqnorm(fit_3b, ~resid(.) | studyfeed)
dev.off() # No evidence of skewness, but s-curve, add histogram for interpretation

pdf("hist_by_group.pdf", width = 8, height = 4.5)
histogram(~ dl_fitted$fit_3b.Res | dl_fitted$studyfeed, xlab = "Residuals")
dev.off() # Both no skewness, slightly lopsided.
...
# It's associated p-value = 0.049

## Sensitivity analysis. Note that the correlation structure is not part of
## the model structure anymore. This is due to convergence issues.
fit_s <- lme(EQ5D.HUS.imp0 ~ studyfeed*fupdaym + EQ5D.HUS.proxy.0 + DEM_SEX +
               APACHEscore + ADM_SYSTEMDIAGNOSIS + ADM_TYPEOPT + NRSscore,
...
# Coefficient of sensitivity analysis, including 95% confidence interval
round(intervals(fit_s, which = "fixed")$fixed[2, ], 3)
# It's associated p-value: p = 0.037
